...
{
    // Ported from Typedown.Core\Services\Transport.cs — routing logic unchanged, only the
    // using directives moved to the new namespace layout. The unused ServiceProvider property
    // from the original (never assigned there either) was dropped.
    public class Transport
    {
        ...
        {
            var msg = JsonConvert.DeserializeObject<EditorMessage>(json, Config.EditorJsonSerializerSettings);
            switch (msg.Type)
            {
                case "invoke":
                    try
