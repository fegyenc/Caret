...
- **Sidebar restructure** (Phase 2 of the warm-autumn reskin, on top of Phase 1's color/icon reskin): a real navigation sidebar above the existing folder tree — `+ New Note`, then Home/Recent/Favorites/All Files/Templates/Trash, matching the brand mockups' layout. Genuinely new features behind three of those, not just new panels on existing data:
  ...
  - Not touched: the top bar (still the plain File/Edit/View menu, not the mockups' search bar + theme/layout icon buttons) and tabs — Caret is still one document per window. Converting that to tabs-in-one-window is a real architectural change (touches `FileViewModel`, per-document dirty/autosave/backup tracking, window-close-confirmation semantics) with real UX tradeoffs against the current multi-window model (e.g. dragging a document to a second monitor), not something to invent unilaterally — left for a follow-up with actual design input.
